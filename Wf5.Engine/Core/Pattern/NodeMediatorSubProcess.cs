...
namespace Wf5.Engine.Core.Pattern
{
    ...
    internal class NodeMediatorSubProcess : NodeMediator
    {
            ...
            try
            {
                if (base.Linker.FromActivity.ActivityType == ActivityTypeEnum.SubProcessNode)
                {
                    ...
                    bool isCompleted = pim.CheckSubProcessInstanceCompleted(Session.Connection,
                        base.Linker.FromActivityInstance.ActivityInstanceID,
            ...
            IDbSession session)
        {
            ...
            //设置活动节点的状态为完成状态
            base.ActivityInstanceManager.Complete(base.Linker.FromActivityInstance.ActivityInstanceID,
            ...
            if (base.Linker.FromActivity.ActivityTypeDetail.ComplexType == ComplexTypeEnum.MultipleInstance)
            {
                ...
                if (base.Linker.FromActivity.ActivityTypeDetail.MergeType == MergeTypeEnum.Sequence)
                {
                    ...
                    if (base.Linker.FromActivityInstance.CompleteOrder < maxOrder)
                    {
                        ...
                        //设置下一个任务对应的子流程进入运行状态
                        base.ProcessInstanceManager.Resume(nextActivityInstance.ActivityInstanceID,
                            activityResource.AppRunner,
                            Session);
           ...
           IDbSession session)
        {
            ...
            else
            {
                ...
                //启动子流程
                WfStartedResult startedResult = null;
                ...
                WfAppRunner subRunner = CopyActivityForwardRunner(activityResource.AppRunner, 
                    ...
                    subProcessNode);
                var runtimeInstance = WfRuntimeManager.CreateRuntimeInstanceStartup(subRunner,
            ...
            IDbSession session)
        {
            ...
            for (short i = 0; i < plist.Count; i++)
            {
                ...
                entity.MIHostActivityInstanceID = toActivityInstance.ActivityInstanceID;
                ...
                //插入活动实例数据，并返回活动实例ID
                entity.ActivityInstanceID = base.ActivityInstanceManager.Insert(entity, session);
                ...
                if ((i > 0) && (toActivity.ActivityTypeDetail.MergeType == MergeTypeEnum.Sequence))
                {
                    var startResult = (WfStartedResult)runtimeInstance.WfExecutedResult;
                    base.ProcessInstanceManager.Suspend(startedResult.ProcessInstanceID, subRunner, subSession);
                }
            }
        }
            ...
            SubProcessNode subProcessNode)
        {
            ...
            subRunner.ProcessGUID = subProcessNode.SubProcessGUID.Value;
            return subRunner;
        }
