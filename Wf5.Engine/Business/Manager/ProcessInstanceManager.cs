...
namespace Wf5.Engine.Business.Manager
{
    ...
    {
        private Repository _processInstanceRepository;
        private Repository ProcessInstanceRepository
        {
            get
            {
                if (_processInstanceRepository == null)
                    _processInstanceRepository = RepositoryFactory.CreateRepository();
        ...
        {
            return ProcessInstanceRepository.GetById<ProcessInstanceEntity>(processInstanceID);
        }
        ...
        internal IEnumerable<ProcessInstanceEntity> GetProcessInstance(string appName,
            ...
            Guid processGUID)
        {
            return ProcessInstanceRepository.Query<ProcessInstanceEntity>(
        ...
        internal ProcessInstanceEntity GetRunningProcessInstance(IDbConnection conn, 
            ...
            Guid processGUID)
        {
            var list = ProcessInstanceRepository.Query<ProcessInstanceEntity>(
                    conn,
                    @"SELECT * FROM WfProcessInstance, 
                                WHERE AppInstanceID=@appInstanceID 
                                    AND ProcessGUID=@processGUID 
        ...
        {
            int newID = ProcessInstanceRepository.Insert(conn, entity, trans);
            entity.ProcessInstanceID = newID;

            Debug.WriteLine(string.Format("process instance inserted, Guid:{0}, time:{1}", entity.ProcessInstanceID.ToString(), System.DateTime.Now.ToString()));
        }
        ...
        internal void Update(ProcessInstanceEntity entity, 
            IDbSession session)
        {
            ProcessInstanceRepository.Update(session.Connection, entity, session.Transaction);
        }
            ...
            ActivityInstanceEntity subProcessNode)
        {
            ...
            {
                //流程的Parent信息
                entity.ParentProcessInstanceID = parentProcessInstance.ProcessInstanceID;
                entity.ParentProcessGUID = parentProcessInstance.ProcessGUID;
                entity.InvokedActivityInstanceID = subProcessNode.ActivityInstanceID;
        ...
        internal bool Discard(WfAppRunner runner, IDbConnection conn=null)
        {
            ...
            using (IDbCommand cmd = conn.CreateCommand())
            {
                try
                {
                    ...
                    int result = ProcessInstanceRepository.ExecuteCommand(cmd);
        ...
        {
            ProcessInstanceEntity entity = ProcessInstanceRepository.GetById<ProcessInstanceEntity>(processInstanceID);
        ...
        internal bool Delete(int processInstanceID)
        {
            ...
            try
            {
                ...
                {
                    ProcessInstanceRepository.Delete<ProcessInstanceEntity>(session.Connection, processInstanceID, session.Transaction);
