...
namespace Wf5.Engine.Data
{
    ...
    public class Repository : IRepository
    {
        ...
        /// <returns></returns>
        public T GetByName<T>(string colName, string value) where T : class
        {
            var tblName = string.Format("dbo.{0}", typeof(T).Name);
            var sql = "SELECT * FROM @table WHERE @columnName = @fieldValue";
            IDbConnection conn = SessionFactory.CreateConnection();
            try
            {
                IEnumerable<T> dataList = SqlMapper.Query<T>(conn, sql, new { table = tblName, columnName = colName, fieldValue = value });
                if (dataList.Count() > 0)
